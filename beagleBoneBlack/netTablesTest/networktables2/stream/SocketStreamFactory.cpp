...
SocketStreamFactory::SocketStreamFactory(const char* _host, int _port):host(_host), port(_port){printf("created SocketStreamFactory at %s:%d\n", host, port);}
...
IOStream* SocketStreamFactory::createStream(){
    ...
    if (server == NULL) {
        printf("ERROR, no such server host %s\n", host);
        return NULL;
    }
