...
void spi_reset() {
	write_spi_register(SPI_RESET_OFFSET, 0x0000000a);
}

void spi_control(int value) {
	write_spi_register(SPI_CONTROL_OFFSET, value);
}

int spi_status() {
	return read_spi_register(SPI_STATUS_OFFSET);
}

void spi_write_data(int data) {
	write_spi_register(SPI_DATA_OUT_OFFSET, data);
}
